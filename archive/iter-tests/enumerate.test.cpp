...
TEST_CASE("Basic Enumeration")
{
    ...
    for (auto [i, e] : v | cxl::adaptors::enumerate(0))
    {
...
TEST_CASE("Enumeration from negative index")
{
    ...
    for (auto [i, e] : v | cxl::adaptors::enumerate(-1))
    {
        REQUIRE(i == ++check);
    }
}
...
TEST_CASE("Enumeration upwards with non-one step")
{
    ...
    for (auto [i, e] : v | cxl::adaptors::enumerate(0, 2))
    {
        REQUIRE(i == check);
        check += 2;
    }
...
TEST_CASE("Enumeration downwards with non-one step")
{
    ...
    for (auto [i, e] : v | cxl::adaptors::enumerate(static_cast<int>(v.size() - 1), -1))
        REQUIRE(i == --check);
}

using namespace cxl::adaptors;
